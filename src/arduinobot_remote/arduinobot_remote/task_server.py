...
class TaskServer(Node):
    def __init__(self):
        ...
        self.arduinobot = MoveItPy(robot_name="moveit_py")
        self.arduinobot_arm = self.arduinobot.get_planning_group("arm")
        self.arduinobot_gripper = self.arduinobot.get_planning_group("gripper")
    ...
    def goalCallback(self, goal_handle):
        ...
        if goal_handle.request.task_number == 0:
            ...
            self.get_logger().error("Action not available.")
            return 
        ...
        if arm_plan_result and gripper_plan_result:
            self.arduinobot_arm.execute(arm_plan_result.trajectory, controllers=["arm_controller"])
            self.arduinobot_gripper.execute(gripper_plan_result.trajectory, controllers=["gripper_controller"])
            
        else:
            self.get_logger().error("Planning failed for task %d." %goal_handle.request.task_number)
            goal_handle.abort()
